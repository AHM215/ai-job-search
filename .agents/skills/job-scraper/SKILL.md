...
3. Read `.claude/skills/job-scraper/search-queries.md` for the canonical search strategy

### Step 1: Search

Read `.claude/skills/job-scraper/search-queries.md` for the canonical search strategy. By default, run the top 3 priority query categories. If the user said "broad", run all categories. If the user specified a focus area (e.g. "data science"), prioritize queries from that category.
...
Use `WebSearch` for:
- Portals listed in `.claude/skills/job-scraper/search-queries.md` that do **not** have a corresponding directory under `.agents/skills/`
- Any portal whose CLI fails at runtime
- When bun is unavailable (Step 1a failed)

Use the site-specific query strings from `.claude/skills/job-scraper/search-queries.md` directly as WebSearch queries for these portals.
